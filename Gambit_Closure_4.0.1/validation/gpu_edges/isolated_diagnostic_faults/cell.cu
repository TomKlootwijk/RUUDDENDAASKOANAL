// TEST-ONLY fault injection; never a production executable.
...
int main(int argc,char** argv){try{
    ...
    auto checked=[&](Engine& state){reference.tick();session.tick(state);if(step==3)state.cells[0].memory^=1u;gambit::compare_states(state.cells,reference.cells,step);
